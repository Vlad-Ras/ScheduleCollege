...
public class CreateModel : PageModel
{
    ...
    {
        Entry.StudyDate = SelectedDate ?? DateTime.Today;
        Entry.Status = "Опубликовано";

        var firstGroup = await _db.Groups.OrderBy(x => x.Code).FirstOrDefaultAsync();
        Entry.GroupId = SelectedGroupId ?? firstGroup?.Id ?? 0;
        Entry.TimeSlotId = SelectedTimeSlotId ?? 0;

        await LoadListsAsync(null, Entry.TeacherId);
    }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        var teacherError = await ValidateTeacherAvailabilityAsync(null, Entry.StudyDate, Entry.TimeSlotId, Entry.TeacherId);
        ...
        var conflict = await FindConflictAsync(null, Entry.StudyDate, Entry.TimeSlotId, Entry.TeacherId, Entry.GroupId, Entry.RoomId);
        ...
        await _audit.AddAsync("Создание занятия", $"ID {Entry.Id}");
